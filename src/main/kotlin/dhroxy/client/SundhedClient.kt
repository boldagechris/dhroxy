...
class SundhedClient(
    ...
    private fun copyForwardedHeaders(incoming: HttpHeaders, outgoing: HttpHeaders) {
        ...
        incoming.forEach { (name, values) ->
            log.info("  {} = {}", name, if (name.lowercase().contains("cookie")) "[REDACTED]" else values)
        }
        log.info("=== END INCOMING HEADERS ===")

        // Map custom X-Sundhed-* headers to their standard counterparts
        // This allows browsers to send Cookie, etc. via non-forbidden header names
        val headerMappings = mapOf(
            "x-sundhed-cookie" to "cookie",
            "x-sundhed-xsrf-token" to "x-xsrf-token",
            "x-sundhed-conversation-uuid" to "conversation-uuid"
        )

        // First, apply any X-Sundhed-* header mappings
        headerMappings.forEach { (customHeader, targetHeader) ->
            val values = incoming[customHeader]
            if (!values.isNullOrEmpty()) {
                log.info("Mapping header {} -> {}", customHeader, targetHeader)
                outgoing.addAll(targetHeader, values)
            }
        }
        ...
        // Always set Accept to application/json for sundhed.dk
        // (sundhed.dk returns HTML if it receives application/fhir+json)
