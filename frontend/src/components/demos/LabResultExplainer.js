...
const LabResultExplainer = () => {
  ...
  const getExplanation = (observation) => {
    ...
    return {
      whatIs: `${testName} er en laboratorietest der måles i dit blod eller urin.`,
      normalRange: fhirRefRange || 'Referenceværdi ikke tilgængelig',
      highMeaning: 'En høj værdi kan have forskellige betydninger afhængigt af din situation.',
      lowMeaning: 'En lav værdi kan have forskellige betydninger afhængigt af din situation.',
      doctorQuestions: [
        'Hvad betyder dette resultat for mig?',
        'Er der behov for yderligere undersøgelser?',
        'Hvornår skal jeg have taget en ny prøve?'
      ],
      relatedTests: [],
